...
app.controller('MainCtrl', [
    '$scope', '$http', 'uiGridConstants', function ($scope, $http, uiGridConstants) {
        ...
        $scope.gridOptions = [];
        //$scope.gridOptions = $scope.commonGridOptions;
        //$scope.gridOptions2 = $scope.commonGridOptions;
        //$scope.gridOptions3 = $scope.commonGridOptions;

        $scope.getRowTemplate = function() {
            return '<div ng-class="{\'row-standard\':true, \'row-target\':row.entity.ResultType===\'Target\', \'row-ending-inventory\':row.entity.ResultType===\'EndingInventory\', \'row-total\':row.entity.ResultType===\'Total\', \'row-adjustment-header\':row.entity.ResultType===\'AdjustmentHeader\', \'row-supply-detail\':row.entity.ResultType===\'SupplyDetail\', \'row-supply-header\':row.entity.ResultType===\'SupplyHeader\' }">'
        ...
        $http.get('/api/searchresults')
            .success(function (data) {
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    data[i].SearchResultDetails.unshift({ "PoolCategory": "Standard", "Notes": "This is a note.", "ResultType": "Title", "Amounts": { "Label": "", "Jan": "Jan", "Feb": "Feb", "Mar": "Mar", "Apr": "Apr", "May": "May", "Jun": "Jun", "Jul": "Jul", "Aug": "Aug", "Sep": "Sep", "Oct": "Oct", "Nov": "Nov", "Dec": "Dec" } });


                    var currentGrade = data[i] === undefined ? '-' : data[i].Grade;
                    $scope.gridOptions[currentGrade] = {};
                    //$scope.gridOptions[currentGrade] = $scope.commonGridOptions;

                    $scope.gridOptions[currentGrade].data = data[i].SearchResultDetails;
                    $scope.gridOptions[currentGrade].enableHorizontalScrollbar = uiGridConstants.scrollbars.NEVER;
                    $scope.gridOptions[currentGrade].enableVerticalScrollbar = uiGridConstants.scrollbars.NEVER;
                    $scope.gridOptions[currentGrade].rowHeight = 20;
                    $scope.gridOptions[currentGrade].rowTemplate = $scope.getRowTemplate();
                    $scope.gridOptions[currentGrade].columnDefs = $scope.getColumnDefs();
                    $scope.gridOptions[currentGrade].onRegisterApi = $scope.onRegisterApi;
                    $scope.gridOptions[currentGrade].columnDefs[0].displayName = currentGrade;
